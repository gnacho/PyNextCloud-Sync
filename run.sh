...
if ! "$python_bin" -c 'import gi; gi.require_version("Gtk", "4.0"); gi.require_version("Adw", "1"); gi.require_version("GdkPixbuf", "2.0"); gi.require_version("Soup", "3.0"); gi.require_version("Secret", "1")' 2>/dev/null; then
    echo "NextSync requires the GNOME Python bindings."
    echo
...
if command -v msgfmt >/dev/null 2>&1; then
    for language in es; do
        ...
        target_file="$target_dir/nextsync.mo"
...
export NEXTSYNC_LAUNCHER="$project_root/run.sh"
export NEXTSYNC_LOCALE_DIR="$locale_dir"
exec "$python_bin" -m nextsync "$@"
