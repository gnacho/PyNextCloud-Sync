...
package locally, copy the `NextSync-0.2.6.zip` and the `PKGBUILD` into a
directory without spaces (makepkg cannot run in paths containing spaces) and run:

```bash
makepkg -cf
sudo pacman -U nextsync-0.2.6-1-any.pkg.tar.zst
```
...
Version `0.2.6` is the current release intended for evaluation. Test it with non-critical data before relying on it for regular synchronization, and always keep independent backups of important files.
