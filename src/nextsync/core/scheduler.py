...
from nextsync.nextcloud.command import NextcloudCmdMissingError, build_command
from nextsync.nextcloud.credentials import KeyringLockedError
from nextsync.storage.config import account_fingerprint
from nextsync.util.paths import config_dir
        ...
        if result.successful and self.delete_alert is None:
            ...
            if guard.get("enabled", True):
                threading.Thread(
                    ...
                    name="nextsync-delete-guard-record",
                    daemon=True,
                ).start()
        try:
